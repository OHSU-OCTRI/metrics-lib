# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [Unreleased]

### Dependencies

- Bumps com.github.eirslett:frontend-maven-plugin from 1.15.4 to 2.0.2 (CIS-3862)

## [0.4.3] - 2026-09-16

### Dependencies

- Run `npm audit fix` to resolve Node.js vulnerabilities (CIS-3824)
- Bumps common_lib from 1.7.1 to 2.1.3 (CIS-3862)
- Bumps authentication_lib from 3.0.0 to 4.3.0 (CIS-3862)

## [0.4.2] - 2026-08-21

### Added

- New actions workflow to update Node.js dependencies on a schedule (CIS-3820)

### Changed

- Use shared GitHub Actions workflows (CIS-3773)
- Create a test build workflow to handle PRs (CIS-3773)

### Dependencies

- Run `npm audit fix` to resolve Node.js vulnerabilities (CIS-3824)

## [0.4.1] - 2026-06-11

### Dependencies

- Use Node.js 24.x for front end tooling (CIS-3719)
- Run `npm audit fix` to resolve Node.js vulnerabilities (CIS-3719)

## [0.4.0] - 2025-12-22

### Added

- Added external logging for interactions (COMPASS-174)

## [0.3.0] - 2025-11-13

### Added

- Add handling to find the closest matching element to populate content
- Favor accessibility attributes over innerText to populate content
- Add an observer to capture DOM elements added after the Interactor is initialized

### Changed

- Rename configurable property from interactionElement to interactionSelector

### Fixed

- Fixed disconnect in Interactor properties
- Truncate interaction content to avoid database persistence errors

## [0.2.0] - 2025-11-06

### Added

- Configurable properties for the interactor (CIS-3396)
- An installation script to streamline use of the interactor (CIS-3396)
- Linting and Prettier config

### Changed

- Set default interactor debug mode to false in the absence of overriding configuration (CIS-3396)

## [0.1.0] - 2025-10-30

### Added

- Initial implementation of the Metrics Library (COMPASS-127)

[unreleased]: https://github.com/OHSU-OCTRI/metrics-lib/compare/v0.4.3...HEAD
[0.4.3]: https://github.com/OHSU-OCTRI/metrics-lib/compare/v0.4.2...v0.4.3
[0.4.2]: https://github.com/OHSU-OCTRI/metrics-lib/compare/v0.4.1...v0.4.2
[0.4.1]: https://github.com/OHSU-OCTRI/metrics-lib/compare/v0.4.0...v0.4.1
[0.4.0]: https://github.com/OHSU-OCTRI/metrics-lib/compare/v0.3.0...v0.4.0
[0.3.0]: https://github.com/OHSU-OCTRI/metrics-lib/compare/v0.2.0...v0.3.0
[0.2.0]: https://github.com/OHSU-OCTRI/metrics-lib/compare/v0.1.0...v0.2.0
[0.1.0]: https://github.com/OHSU-OCTRI/metrics-lib/compare/b398fac2038a635d3bc7322f004fbf78d6f1bbfd...v0.1.0
